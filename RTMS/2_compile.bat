...
javac -cp "%CP%" -d out %SRC%\model\User.java %SRC%\model\Driver.java %SRC%\model\Staff.java %SRC%\model\RaceEvent.java %SRC%\model\RaceEntry.java %SRC%\model\Sponsor.java %SRC%\model\BudgetTransaction.java
if %ERRORLEVEL% NEQ 0 ( echo [FAILED] model & pause & exit /b 1 )
echo [OK] model

echo [INFO] Compiling dao...
javac -cp "%CP%;out" -d out %SRC%\dao\UserDAO.java %SRC%\dao\DriverDAO.java %SRC%\dao\StaffDAO.java %SRC%\dao\RaceEventDAO.java %SRC%\dao\SponsorDAO.java %SRC%\dao\BudgetDAO.java
if %ERRORLEVEL% NEQ 0 ( echo [FAILED] dao & pause & exit /b 1 )
echo [OK] dao

echo [INFO] Compiling main...
javac -cp "%CP%;out" -d out %SRC%\main\RTMSApplication.java
